...
const login = async (req, res) => {
  ...
  redirect(
    res,
    302,
    `https://${githubUrl}/login/oauth/authorize?client_id=${process.env
      .GH_CLIENT_ID}&state=${state}`
  )
}
...
const callback = async (req, res) => {
  ...
  if (!code && !state) {
    redirectWithQueryString(res, {
      error: 'Provide code and state query parameter'
    })
    ...
    try {
      ...
      redirectWithQueryString(res, {
        error:
          'Please provide GH_CLIENT_ID and GH_CLIENT_SECRET as environment variables. If this continues to fail, check if GitHub is experiencing a service outage.'
      })
    }
  }
}
